...
for i in range(16):
    if i%4 == 0:
        ...
        s = output[i//4][1]
        norm = {'type' : s}
        f, b, l_d = run_exp_depresion(i+1, train, test, labels_dep, test_labels,chose =2, tau=0.98, remove_stop=k,
...
for i in range(16):
    if i%4 == 0:
        ...
        s = output[i//4][1]
        norm = {'type' : s}
        f, b, l_d = run_exp_depresion(i+17, train, test, labels_dep, test_labels,chose =2, tau=0.95, remove_stop=k,
                                  name_dict='dict5',norm_data=True, sub_param=norm)
        f_score.append(f)
        best.append(b)
        norm_dataset.append(s)
...
df.to_csv('/home/est_posgrado_maria.garcia/Proyecto_tecnologico/Results/Depresion_fuzzy/All_result_fuzzy_dep1.csv',sep='\t')
...
#    f.write(dfAsString)
#    f.close()
df.to_csv('/home/est_posgrado_maria.garcia/Proyecto_tecnologico/Results/Depresion_fuzzy/Best_Result_fuzzy_dep1.csv',sep='\t')
