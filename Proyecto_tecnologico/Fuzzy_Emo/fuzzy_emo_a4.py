...
for i in range(16):
    if i%4 == 0:
        ...
        s = output[i//4][1]
        norm = {'type' : s}
        f, b, l_d = run_exp_anorexia(i+49, tr_anorexia, test_anxia, tr_label, test_labels_anxia, chose =4, tau=0.98, remove_stop=k,
                                  name_dict='dict1',norm_data=True, only_bow = False,sub_param=norm)
        f_score.append(f)
        best.append(b)
    ...
    if i%4==1:
        ...
        norm = {'type' : s}
        f, b, l_d = run_exp_anorexia(i+49, tr_anorexia, test_anxia, tr_label, test_labels_anxia, chose = 4,tau=0.98, remove_stop=k,
                                  name_dict='dict2',norm_data=True, only_bow = False,sub_param=norm)
        f_score.append(f)
        best.append(b)
        norm_dataset.append(s)
    ...
    if i%4==2:
        ...
        norm = {'type' : s}
        f, b, l_d = run_exp_anorexia(i+49, tr_anorexia, test_anxia, tr_label, test_labels_anxia, chose =4, tau=0.98, remove_stop=k,
                                  name_dict='dict3',norm_data=True,only_bow = False, sub_param=norm)
        f_score.append(f)
        best.append(b)
        norm_dataset.append(s)
    ...
    if i%4==3:
        ...
        norm = {'type' : s}
        f, b, l_d = run_exp_anorexia(i+49, tr_anorexia, test_anxia, tr_label, test_labels_anxia, chose =4,tau=0.98, remove_stop=k,
                                  name_dict='dict4',norm_data=True, only_bow =False,sub_param=norm)
        f_score.append(f)
        best.append(b)
        norm_dataset.append(s)
...
df.to_csv('/home/est_posgrado_maria.garcia/Proyecto_tecnologico/Results/Anorexia_fuzzy_emo/result_fuzzy_anxia4.csv',sep='\t')
