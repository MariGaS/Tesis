...
for i in range(16):
    if i%4 == 0:
        ...
        s = output[i//4][1]
        norm = {'type' : s}
        f, b, l_d = run_exp_depresion(i+33, train, test, labels_dep, test_labels, chose =4,tau=0.98, remove_stop=k,
                                  name_dict='dict5',norm_data=True, only_bow = True,sub_param=norm)
        f_score.append(f)
        best.append(b)
    ...
    if i%4==1:
        ...
        norm = {'type' : s}
        f, b, l_d = run_exp_depresion(i+33, train, test, labels_dep, test_labels, chose =4,tau=0.98, remove_stop=k,
                                  name_dict='dict6',norm_data=True, only_bow = True,sub_param=norm)
        f_score.append(f)
        remove.append(k)
        best.append(b)
        norm_dataset.append(s)
    ...
    if i%4==2:
        ...
        norm = {'type' : s}
        f, b, l_d = run_exp_depresion(i+33, train, test, labels_dep, test_labels,chose =4, tau=0.98, remove_stop=k,
                                  name_dict='dict7',norm_data=True, only_bow = True,sub_param=norm)
        f_score.append(f)
        best.append(b)
        norm_dataset.append(s)
    ...
    if i%4==3:
        ...
        norm = {'type' : s}
        f, b, l_d = run_exp_depresion(i+33, train, test, labels_dep, test_labels,chose =4, tau=0.98, remove_stop=k,
                                  name_dict='dict8',norm_data=True, only_bow = True,sub_param=norm)
        f_score.append(f)
        best.append(b)
        norm_dataset.append(s)
...
for i in range(16):
    if i%4 == 0:
        ...
        s = output[i//4][1]
        norm = {'type' : s}
        f, b, l_d = run_exp_depresion(i+48, train, test, labels_dep, test_labels,chose =4, tau=0.95, remove_stop=k,
                                  name_dict='dict5',norm_data=True, only_bow = True,sub_param=norm)
        f_score.append(f)
        best.append(b)
        norm_dataset.append(s)
...
df.to_csv('/home/est_posgrado_maria.garcia/Proyecto_tecnologico/Results/Depresion_fuzzy_emo/Result_fuzzy_emo_dep2.csv',sep='\t')
