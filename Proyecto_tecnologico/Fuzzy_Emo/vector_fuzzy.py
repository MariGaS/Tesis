...
def classificator_fuzzy(data, test, name_dic, tau, chose,remove_stop=False, norm_data=False,only_bow = True, sub_param={}):
    # número de sujetos
    num_doc = len(data)
    num_test = len(test)
...
def run_exp_anorexia(num_exp, train_data, test_data, y_train, y_test, chose,tau, remove_stop, name_dict, norm_data, only_bow, sub_param = {}):
    seed_val = 42
    np.random.seed(seed_val)
    print('Experiment:', num_exp )
    t_initial = time()    
    parameters = {'C': [.05, .12, .25, .5, 1, 2, 4]}
    x_train, x_test, l_d = classificator_fuzzy(train_data, test_data,tau=tau,chose=chose,remove_stop=remove_stop,
                                               name_dic=name_dict, norm_data=norm_data,only_bow= only_bow, sub_param=sub_param)
    if only_bow == True: 
    ...
    path_name = '/home/est_posgrado_maria.garcia/Proyecto_tecnologico/Results/Anorexia_fuzzy_emo' + '/' + result_name
...
def run_exp_depresion(num_exp, train_data, test_data, y_train, y_test, chose, tau, remove_stop, name_dict, norm_data, only_bow, sub_param={}):
    print('Experiment: ', num_exp)
    parameters = {'C': [.05, .12, .25, .5, 1, 2, 4]}
    seed_val = 42
    np.random.seed(seed_val)
    ...
    path_name = '/home/est_posgrado_maria.garcia/Proyecto_tecnologico/Results/Depresion_fuzzy_emo' + '/' + result_name
