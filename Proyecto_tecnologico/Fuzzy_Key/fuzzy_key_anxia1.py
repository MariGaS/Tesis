...
for i in range(12):
    f = run_exp_anxia_sim(i+1, all_pos, all_neg,test_anxia,test_labels_anxia, tr_label,arg1[i],arg2[i],tau=arg3[i],
                            chose =1,dif = m[i], fuzzy= arg5[i],remove_stop=arg6[i],train_data = False, compress=False, concatenate = True)
    f_score.append(f)

    
for i in range(12):
    f = run_exp_anxia_sim(i+13, all_pos, all_neg,test_anxia,test_labels_anxia, tr_label,arg1[i],arg2[i],tau=arg3[i],
                            chose =1,dif = m[i], fuzzy= arg5[i],remove_stop=arg7[i],train_data = False, compress=False, concatenate = True)
    f_score.append(f)
#en este si importa 
for i in range(12):
    f = run_exp_anxia_sim(i+25, all_pos, all_neg,test_anxia,test_labels_anxia, tr_label,arg1[i],arg2[i],tau=arg3[i],
                            chose =1,dif = n[i], fuzzy= arg5[i],remove_stop=arg6[i],train_data = False, compress=False, concatenate = True)
    f_score.append(f)


for i in range(12):
    f= run_exp_anxia_sim(i+37, all_pos, all_neg,test_anxia,test_labels_anxia, tr_label,arg1[i],arg2[i],tau=arg3[i],
...
df.to_csv('/home/est_posgrado_maria.garcia/Proyecto_tecnologico/Results/Anorexia_fuzzy_key/result_fuzzy_key_anxia1.csv',sep='\t')
