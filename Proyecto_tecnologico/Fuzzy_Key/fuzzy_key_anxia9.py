...
for i in range(12):
    f,a = run_exp_anxia_sim(i+1097, all_pos, all_neg,test_anxia,test_labels_anxia, tr_label,arg1[i],arg2[i],tau=arg3[i],
                            chose =3,dif = m[i], fuzzy= arg5[i],remove_stop=arg6[i],train_data = True, compress=False, concatenate = True)
    f_score.append(f)
    best.append(a)
    
for i in range(12):
    f,a = run_exp_anxia_sim(i+1109, all_pos, all_neg,test_anxia,test_labels_anxia, tr_label,arg1[i],arg2[i],tau=arg3[i],
                            chose =3,dif = m[i], fuzzy= arg5[i],remove_stop=arg7[i],train_data = True, compress=False, concatenate = True)
    f_score.append(f)
    best.append(a)
#en este si importa 
for i in range(12):
    f,a = run_exp_anxia_sim(i+1121, all_pos, all_neg,test_anxia,test_labels_anxia, tr_label,arg1[i],arg2[i],tau=arg3[i],
                            chose =3,dif = n[i], fuzzy= arg5[i],remove_stop=arg6[i],train_data = True, compress=False, concatenate = True)
    f_score.append(f)
    best.append(a)

for i in range(12):
    f,a = run_exp_anxia_sim(i+1133, all_pos, all_neg,test_anxia,test_labels_anxia, tr_label,arg1[i],arg2[i],tau=arg3[i],
...
df.to_csv('/home/est_posgrado_maria.garcia/Proyecto_tecnologico/Results/Anorexia_fuzzy_key/result_fuzzy_key_anxia9.csv',sep='\t')
