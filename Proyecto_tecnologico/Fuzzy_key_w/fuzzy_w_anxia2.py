...
for i in range(12):
    f = run_exp_anxia_sim(i+49, all_pos, all_neg,test_anxia,test_labels_anxia, tr_label,arg1[i],arg2[i],tau=arg3[i], w = arg8[i],
                            chose =1,dif = m[i], fuzzy= arg5[i],remove_stop=arg6[i],train_data = False, compress=False, concatenate = False)
    f_score.append(f)

    
for i in range(12):
    f = run_exp_anxia_sim(i+61, all_pos, all_neg,test_anxia,test_labels_anxia, tr_label,arg1[i],arg2[i],tau=arg3[i], w= arg8[i],
                            chose =1,dif = m[i], fuzzy= arg5[i],remove_stop=arg7[i],train_data = False, compress=False, concatenate = False)
    f_score.append(f)
#en este si importa 
for i in range(12):
    f,a = run_exp_anxia_sim(i+73, all_pos, all_neg,test_anxia,test_labels_anxia, tr_label,arg1[i],arg2[i],tau=arg3[i], w = arg8[i],
                            chose =1,dif = n[i], fuzzy= arg5[i],remove_stop=arg6[i],train_data = False, compress=False, concatenate = False)
    f_score.append(f)


for i in range(12):
    f,a = run_exp_anxia_sim(i+85, all_pos, all_neg,test_anxia,test_labels_anxia, tr_label,arg1[i],arg2[i],tau=arg3[i], w = arg8[i],
