...
for i in range(12):
    f,a,len1,len2 = run_exp_anxia_sim(i+241, all_pos, all_neg,test_anxia,test_labels_anxia, tr_label,arg1[i],arg2[i],tau=arg3[i], w = arg8[i],
                            chose =1,dif = m[i], fuzzy= arg5[i],remove_stop=arg6[i],train_data = True, compress=False, concatenate = False)
    f_score.append(f)
    best.append(a)
...
for i in range(12):
    f,a,len1,len2 = run_exp_anxia_sim(i+253, all_pos, all_neg,test_anxia,test_labels_anxia, tr_label,arg1[i],arg2[i],tau=arg3[i], w = arg8[i],
                            chose =1,dif = m[i], fuzzy= arg5[i],remove_stop=arg7[i],train_data = True, compress=False, concatenate = False)
    f_score.append(f)
    best.append(a)
    dic1_len.append(len1)
    dic2_len.append(len2)
#en este si importa 
for i in range(12):
    f,a,len1,len2 = run_exp_anxia_sim(i+265, all_pos, all_neg,test_anxia,test_labels_anxia, tr_label,arg1[i],arg2[i],tau=arg3[i],w=arg8[i],
                            chose =1,dif = n[i], fuzzy= arg5[i],remove_stop=arg6[i],train_data = True, compress=False, concatenate = False)
    f_score.append(f)
    best.append(a)
    dic1_len.append(len1)
    dic2_len.append(len2)
...
for i in range(12):
    f,a,len1,len2 = run_exp_anxia_sim(i+277, all_pos, all_neg,test_anxia,test_labels_anxia, tr_label,arg1[i],arg2[i],tau=arg3[i],w = arg8[i],
                            chose =1,dif = n[i], fuzzy= arg5[i],remove_stop=arg7[i],train_data = True, compress=False, concatenate = False)
    f_score.append(f)
    best.append(a)
    dic1_len.append(len1)
    dic2_len.append(len2)
...
con = [False]*48
cho = [1]*48
l = [str(x) for x in range(241,289)]
data = { 'score1':score1, 'score2':score2,  'tol': tol, 'remove_stop':rem,'fuzzy':vec,'no_dist': n_d, 
            ...
            'concatenate': con, 'word_emebeddings':cho}
df = pd.DataFrame(data, index= l)


df.to_csv('/home/est_posgrado_maria.garcia/Tesis/Proyecto_tecnologico/Results/Anorexia_w_key/Result_w_anxia6.csv',sep='\t')
