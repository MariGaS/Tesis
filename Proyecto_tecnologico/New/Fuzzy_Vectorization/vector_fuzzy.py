...
def classificator_fuzzy(all_path_train, all_path_test, path_tf_train, path_tf_test, num_test, num_train,
                        name_dic, tau, chose,fuzzy = True,tf= True):
    # número de sujetos
...
def run_exp_anxia_sim(num_exp, test_labels, train_labels, num_test, num_train,name_dic,
                    chose, tau,fuzzy, remove_stop, tf, classificator):
    print(num_exp)
    ...
    X_test,X_train,len_dict = classificator_fuzzy(all_path_train = path_train, all_path_test =path_test,
                    ...
                    name_dic = name_dic, tau = tau, chose=chose,fuzzy = fuzzy,tf= tf)	
    ...
    if classificator == 'NB':
        a = 'None'
    f = open('/home/est_posgrado_maria.garcia/Tesis/Proyecto_tecnologico/New/Fuzzy_Vectorization/f1_anorexia.txt','a')
    f.write('\n' + str(num_exp) + ',' + str(tau)  
            +','+ fuzzy_str+','+ remove_stop_str 
            +','+ w_e + ','+ weight + ',' + name_dic+  ',' + str(len_dict) +  ',' + classificator + ','+ str(f1) + ',' + str(a)) 
    f.close()   
...
def run_exp_dep_sim(num_exp, test_labels, train_labels, num_test, num_train,name_dic,
    ...
    f = open('/home/est_posgrado_maria.garcia/Tesis/Proyecto_tecnologico/New/Fuzzy_Vectorization/f1_dep.txt','a')
    f.write('\n' + str(num_exp) + ',' + str(tau)  
            +','+ fuzzy_str+','+ remove_stop_str 
            +','+ w_e + ','+ weight + ',' + name_dic+  ',' + str(len_dic) +  ',' + classificator + ','+ str(f1) + ',' + str(a)) 
    f.close()   
