...
def classificator_pos_neg(all_path_train, all_path_test, num_test, num_train, score1, score2, di1, di2, tau, chose, groups, dif=True,
   ...
   ### For using svm ####
    ...
    if compress == True:
        ...
        #number of elements by groups 
        num_by_groups = score2//groups
        if score2%groups == 0:
            left = 0
            right = num_by_groups-1
            for i in range(groups):

                X_temp = np.sum(X_test_[:,left:right], axis = 1)
                X_test2 = np.column_stack((X_test2, X_temp))
        ...
        if score2%groups == 0:
            ...
            for i in range(groups):
                ...
                X_temp = np.sum(X_train_[:,left:right], axis = 1)
                X_train2 = np.column_stack((X_train2, X_temp))
    ...
    #no compress 
        ...
        if score2%groups == 0:
            ...
            right += score2%groups
            X_temp = np.sum(X_train_[:,left:right], axis = 1)
            X_train2 = np.concatenate((X_train2, X_temp), axis = 1)         
        ...
        if score2%groups == 0:
            ...
            right += score2%groups
            X_temp = np.sum(X_test_[:,left:right], axis = 1)
            X_test2 = np.concatenate((X_test2, X_temp), axis = 1)            
...
def run_exp_anxia_sim(num_exp, test_labels, train_labels, num_test, num_train,score1, score2,
                      chose, tau, groups, dif, fuzzy, remove_stop, compress, dic):
    print(num_exp)
    logging.basicConfig(filename="/home/est_posgrado_maria.garcia/Tesis/Proyecto_tecnologico/New/fuzzy_key_range/warnings_anorexia.txt",level=logging.DEBUG)
    ...
    X_test,X_train,len_dic1,len_dic2 = classificator_pos_neg(path_train, path_test, num_test, num_train,score1,score2,kw1,kw2, 
        			tau=tau,chose=chose,groups = groups, dif = dif, fuzzy = fuzzy, compress=compress, con = con)	
        
    if X_test.shape[1] < X_test.shape[0]:
        svr = svm.LinearSVC(class_weight='balanced', dual=False, max_iter = 4000)
        
    else: 
        svr = svm.LinearSVC(class_weight='balanced', dual=True, max_iter = 4000)
    ...
    f = open('/home/est_posgrado_maria.garcia/Tesis/Proyecto_tecnologico/New/fuzzy_key_range/f1_anorexia.txt','a')
    f.write('\n' + str(num_exp) + ',' + str(score1) + ',' + str(score2) +',' + str(tau)+','+ str(groups) +',' + str(dif) 
...
def run_exp_dep_sim(num_exp,  test_labels, train_labels,num_test,num_train, score1, score2,
    ...
    logging.basicConfig(filename="/home/est_posgrado_maria.garcia/Tesis/Proyecto_tecnologico/New/fuzzy_key_range/warnings_dep.txt",level=logging.DEBUG)
    logging.debug('\n This is a message from experimet number ' + str(num_exp) )
    logging.info('\n This is a message for info')
    logging.captureWarnings(True)
    logging.warning('\nWarning message for experiment number ' + str(num_exp))
    ...
    f1 = f1_score(test_labels, y_pred)
    f = open('/home/est_posgrado_maria.garcia/Tesis/Proyecto_tecnologico/New/fuzzy_key_range/f1_dep.txt','a')
    f.write('\n' + str(num_exp) + ',' + str(score1) + ',' + str(score2) +',' + str(tau)+','+ str(groups) +',' + str(dif) 
            +','+ str(fuzzy) +','+ str(remove_stop) +','+ str(compress)+  ',' + str(chose) + ','+ str(dic) + ',' + str(f1) + ',' + str(a)) 
    f.close()       
