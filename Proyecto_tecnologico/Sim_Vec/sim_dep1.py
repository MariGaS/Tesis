...
for i in range(20):
    if i < 10:
        ...
        f= run_exp_dep_sim(i+1, train_pos, train_neg,test,test_labels,train_y, num_feats[i-10],num_feats[i-10], tau=0.99,
...
for i in range(20):
    if i < 10:
        f = run_exp_dep_sim(i+21, train_pos, train_neg,test,test_labels,train_y, num_feats[i],num_feats[i], tau=0.99,
                                 chose =2,fuzzy = False, remove_stop=False,train_data = False, compress=False)
        f_score.append(f)
        tol.append(0.99)
        n_feat.append(num_feats[i])
        l5.append('False')
        ...
        f= run_exp_dep_sim(i+21, train_pos, train_neg,test,test_labels,train_y, num_feats[i-10],num_feats[i-10], tau=0.99,
                                chose =2,fuzzy = False, remove_stop=True,train_data = False, compress=False)
        f_score.append(f)
        tol.append(0.99)
        n_feat.append(num_feats[i-10])
        l5.append('True')
...
df.to_csv('/home/est_posgrado_maria.garcia/Proyecto_tecnologico/Results/Depresion_sim/All_result_sim_dep1.csv', sep='\t')
...
df.to_csv('/home/est_posgrado_maria.garcia/Proyecto_tecnologico/Results/Depresion_sim/Best_Result_sim_dep1.csv', sep='\t')
