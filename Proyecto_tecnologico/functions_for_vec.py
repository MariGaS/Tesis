...
#modelo de anorexia entrenado en fastext 
print('Load_anxia')
model_anxia = FastText.load('Proyecto_tecnologico/Model/anxiety.model')
#modelo de depresion entrenado
print('Load_dep')
model_dep   = FastText.load('Poryecto_tecnologico/Model/depresion.model')
#modelo pre-entrenado de fasttext 
#model_pre = gensim.downloader.load('fasttext-wiki-news-subwords-300')
...
def get_dictionary(name_dic):
    if name_dic == 'dict29':
        vocab = []
        with open('vocabulary_ex.txt', 'r') as f:
            for twitt in f:
        ...
        vocab = []
        with open('vocabulary_extending.txt', 'r') as f:
            for twitt in f:
                vocab.append(twitt[:-1])
            f.close()
        vocab_plus = set(vocab)
        ...
        vocab = []
        with open('vocab_extending51.txt', 'r') as f:
            for twitt in f:
                vocab.append(twitt[:-1])
            f.close()
        vocab_plus = set(vocab)
        ...
        vocab = []
        with open('vocab_ex51.txt', 'r') as f:
            for twitt in f:
                vocab.append(twitt[:-1])
            f.close()
        vocab_plus = set(vocab)
...
def get_vec_rep(words_doc, dictionary, option, norm = False, param_norm = {}):
    ...
    for i in range(words_user.shape[0]):
        ...
        if option == 1: 
        	words_user[i] = model_anxia[w1]
        elif option == 2: 
        	words_user[i] = model_dep[w1]
        #elif option == 3: 
        #	words_user[i] = model_dep[w1]
...
def run_exp_anorexia(num_exp, train_data, test_data, y_train, y_test,option,name_dict, norm_data, norm_vec, sub_param = {}, param_norm={}):
    parameters = {'C': [.05, .12, .25, .5, 1, 2, 4]}
    seed_val = 42

    np.random.seed(seed_val)

    x_train, x_test = classificator_vectors(train_data, test_data, name_dic=name_dict,option, norm_vec=norm_vec,
    ...
    path_name = 'Proyecto_tecnologico/Results/Anorexia_vec' + '/' + result_name
...
def run_exp_dep(num_exp, train_data, test_data, y_train, y_test,option,name_dict, norm_data, norm_vec, sub_param = {}, param_norm={}):
    parameters = {'C': [.05, .12, .25, .5, 1, 2, 4]}
    seed_val = 42
    ...
    path_name = 'Proyecto_tecnologico/Results/Depresion_vec' + '/' + result_name
